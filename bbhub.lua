...
AuraHub.Info = {Version="2.0.1"}
AuraHub.Save = {UISize={550,380},TabSize=100,Theme="Darker"}
AuraHub.ConfigData = {}
AuraHub.ControlRegistry = {}
AuraHub.Localization = {}
AuraHub.CurrentLang = "en"
AuraHub.UserRole = "member"
AuraHub.UndoStack = {}
AuraHub.RedoStack = {}
AuraHub.Sounds = {
	ButtonClick = 0,
...
local function getCurrentTheme(c)
	if type(c)=="table" then
		...
		return AuraHub.Themes[c or AuraHub.Save.Theme]
	end
end
...
function AuraHub:RegisterControl(k,u)
	AuraHub.ControlRegistry[k]=u
end

function AuraHub:ApplyConfig(cf)
	for k,v in pairs(cf) do
		if AuraHub.ControlRegistry[k] then
			AuraHub.ControlRegistry[k](v)
		end
	end
end

function AuraHub:SetUserRole(roleName)
	self.UserRole = roleName
end

function AuraHub:CheckRole(requiredRole)
	return self.UserRole==requiredRole
end

function AuraHub:AddUndo(controlKey,oldValue,newValue)
	table.insert(self.UndoStack,{key=controlKey,old=oldValue,new=newValue})
end

function AuraHub:Undo()
...
function AuraHub:RegisterTheme(name,definition)
	self.Themes[name] = definition
end

function AuraHub:PlaySound(name)
...
function AuraHub:SetLanguage(lang)
	self.CurrentLang=lang
end

function AuraHub:LocalizeText(textKey)
...
function SectionMethods:addMenu(n)
	...
	local f = createInstance("Frame", {
		...
		BackgroundColor3 = getCurrentTheme(AuraHub.Save.Theme)["Color Stroke"],
		BackgroundTransparency = 0.3
	}, self.Content)
	createInstance("UICorner", {CornerRadius = UDim.new(0, 6)}, f)
	createInstance("UIGradient", {
		Color = getCurrentTheme(AuraHub.Save.Theme)["Color Hub 1"]
	}, f)
	...
	local label = createInstance("TextLabel", {
		...
		Size = UDim2.new(1, 0, 1, 0),
		BackgroundTransparency = 1,
		TextColor3 = getCurrentTheme(AuraHub.Save.Theme)["Color Text"],
		Font = Enum.Font.GothamBold,
		TextSize = 16,
		TextXAlignment = Enum.TextXAlignment.Left
	}, f)
...
function SectionMethods:addToggle(o)
	...
	if role and not AuraHub:CheckRole(role) then
	...
	addBorder(f,getCurrentTheme(AuraHub.Save.Theme)["Color Stroke"],1)
	local l=createInstance("TextLabel",{Size=UDim2.new(0.7,0,1,0),Position=UDim2.new(0,0,0,0),BackgroundTransparency=1,Text=t,TextColor3=getCurrentTheme(AuraHub.Save.Theme)["Color Text"],Font=Enum.Font.Gotham,TextSize=12,TextXAlignment=Enum.TextXAlignment.Left},f)
	...
	AuraHub.ConfigData[t]=s
	AuraHub:RegisterControl(t,function(v)
		local old=s
		s=v
		if s then
		...
		AuraHub:AddUndo(t,old,s)
	end)
	sw.InputBegan:Connect(function(i)
		if i.UserInputType==Enum.UserInputType.MouseButton1 or i.UserInputType==Enum.UserInputType.Touch then
			AuraHub:PlaySound("ButtonClick")
			local old=s
			if not s then
...
function SectionMethods:addSlider(o)
	...
	if AuraHub.ConfigData[n] ~= nil then
		df = AuraHub.ConfigData[n]
	else
		AuraHub.ConfigData[n] = df
	end
	local cb = o.Callback or function(x) end
	local TweenService = game:GetService("TweenService")
	...
	local sh = createInstance("Frame", {
		...
		BackgroundColor3 = getCurrentTheme(AuraHub.Save.Theme)["Color Theme"],
	...
	sh.InputEnded:Connect(function(i)
		if i.UserInputType == Enum.UserInputType.MouseButton1 or i.UserInputType == Enum.UserInputType.Touch then
			drag = false
			local currentValue = AuraHub.ConfigData[n]
	...
	UserInputService.InputChanged:Connect(function(i)
		if drag and (i.UserInputType == Enum.UserInputType.MouseMovement or i.UserInputType == Enum.UserInputType.Touch) then
			...
			AuraHub.ConfigData[n] = val
			cb(val)
		end
	end)
	AuraHub:RegisterControl(n, function(sv)
	...
	vl.FocusLost:Connect(function(enterPressed)
		...
		if newVal then
			...
			AuraHub.ConfigData[n] = newVal
			cb(newVal)
		else
			vl.Text = tostring(math.floor(AuraHub.ConfigData[n]))
		end
	end)
	return f
end
...
function SectionMethods:addTextBox(o)
	...
	addBorder(f, getCurrentTheme(AuraHub.Save.Theme)["Color Stroke"], 1)
	local l = createInstance("TextLabel", {Name = "Label", Text = n, Size = UDim2.new(0.7, 0, 1, 0), Position = UDim2.new(0, 0, 0, 0), BackgroundTransparency = 1, TextColor3 = getCurrentTheme(AuraHub.Save.Theme)["Color Text"], Font = Enum.Font.Gotham, TextSize = 10}, f)
	local tb = createInstance("TextBox", {Name = "Input", Text = d, Size = UDim2.new(0.3, 0, 1, 0), Position = UDim2.new(0.7, 0, 0, 0), BackgroundColor3 = getCurrentTheme(AuraHub.Save.Theme)["Color Stroke"], TextColor3 = getCurrentTheme(AuraHub.Save.Theme)["Color Text"], Font = Enum.Font.Gotham, TextSize = 10, TextWrapped = true}, f)
	tb.FocusLost:Connect(function(e)
		AuraHub.ConfigData[n] = tb.Text
		cb(tb.Text)
	end)
	AuraHub:RegisterControl(n, function(sv)
		tb.Text = sv
...
function SectionMethods:addDropdown(o)
	...
	createInstance("UICorner", {CornerRadius = UDim.new(0,20)}, f)
	addBorder(f, getCurrentTheme(AuraHub.Save.Theme)["Color Stroke"], 1)
	local l = createInstance("TextLabel", {Name = "Label", Text = t, Size = UDim2.new(0.7, 0, 0, ch), Position = UDim2.new(0, 5, 0, 0), BackgroundTransparency = 1, TextColor3 = getCurrentTheme(AuraHub.Save.Theme)["Color Text"], Font = Enum.Font.GothamBold, TextSize = 12, TextXAlignment = Enum.TextXAlignment.Left}, f)
	local b = createInstance("TextButton", {Name = "DropdownButton", Text = (df ~= "" and (df.." ▼") or "Select ▼"), Size = UDim2.new(0.3, 0, 0, ch), Position = UDim2.new(0.7, -5, 0, 0), BackgroundTransparency = 0, BackgroundColor3 = getCurrentTheme(AuraHub.Save.Theme)["Color Stroke"], Font = Enum.Font.GothamBold, TextSize = 12, TextColor3 = getCurrentTheme(AuraHub.Save.Theme)["Color Text"]}, f)
	createInstance("UICorner", {CornerRadius = UDim.new(0,20)}, b)
	local lf = createInstance("Frame", {Name = "DropdownList", BackgroundTransparency = 0, BackgroundColor3 = getCurrentTheme(AuraHub.Save.Theme)["Color Hub 2"], Visible = false, Position = UDim2.new(0, 0, 0, ch)}, f)
	...
	b.MouseButton1Click:Connect(function()
		if dt then
			...
			tweenProperty(b, {TextColor3 = getCurrentTheme(AuraHub.Save.Theme)["Color Text"]}, 0.2)
	...
	local function populateItems()
		...
		for _, op in ipairs(it) do
			...
			addBorder(btn, getCurrentTheme(AuraHub.Save.Theme)["Color Stroke"], 1)
			btn.MouseButton1Click:Connect(function()
				l.Text = t.." - "..op
				AuraHub.ConfigData[t] = op
				pcall(cb, op)
				tweenProperty(f, {Size = UDim2.new(1, 0, 0, ch)}, 0.2)
				dt = false
				wait(0.2)
	...
	if #presets > 0 then
		for _, preset in ipairs(presets) do
			...
			addBorder(pbtn, getCurrentTheme(AuraHub.Save.Theme)["Color Stroke"], 1)
			pbtn.MouseButton1Click:Connect(function()
				l.Text = t.." - "..preset
				AuraHub.ConfigData[t] = preset
				pcall(cb, preset)
				tweenProperty(f, {Size = UDim2.new(1, 0, 0, ch)}, 0.2)
				dt = false
				wait(0.2)
	...
	local resetButton = createInstance("TextButton", {Name = "ResetButton", Text = "↺", Size = UDim2.new(1, 0, 1, 0), BackgroundTransparency = 0, BackgroundColor3 = Color3.fromRGB(40,40,40), Font = Enum.Font.GothamBold, TextSize = 12, TextColor3 = getCurrentTheme(AuraHub.Save.Theme)["Color Text"]}, rightIcons)
	createInstance("UICorner", {CornerRadius = UDim.new(0,5)}, resetButton)
	resetButton.MouseButton1Click:Connect(function()
		l.Text = t.." - "..df
		AuraHub.ConfigData[t] = df
		pcall(cb, df)
		tweenProperty(f, {Size = UDim2.new(1, 0, 0, ch)}, 0.2)
		dt = false
		wait(0.2)
	...
	AuraHub.ConfigData[t] = df
	AuraHub:RegisterControl(t, function(sv)
		l.Text = t.." - "..sv
	end)
	local upd = {}
	function upd:Clear()
...
function SectionMethods:addColorPicker(o)
	...
	addBorder(f,getCurrentTheme(AuraHub.Save.Theme)["Color Stroke"],1)
	local l=createInstance("TextLabel",{Name="Label",Text=n,Size=UDim2.new(1,0,0,20),BackgroundTransparency=1,TextColor3=getCurrentTheme(AuraHub.Save.Theme)["Color Text"],Font=Enum.Font.Gotham,TextSize=10,TextXAlignment=Enum.TextXAlignment.Left},f)
	...
	local r=createInstance("TextBox",{Name="R",Text=tostring(math.floor(d.R*255)),Size=UDim2.new(0,30,0,20),Position=UDim2.new(0,5,0,25),BackgroundColor3=getCurrentTheme(AuraHub.Save.Theme)["Color Stroke"],TextColor3=getCurrentTheme(AuraHub.Save.Theme)["Color Text"],Font=Enum.Font.Gotham,TextSize=10},f)
	local g=createInstance("TextBox",{Name="G",Text=tostring(math.floor(d.G*255)),Size=UDim2.new(0,30,0,20),Position=UDim2.new(0,40,0,25),BackgroundColor3=getCurrentTheme(AuraHub.Save.Theme)["Color Stroke"],TextColor3=getCurrentTheme(AuraHub.Save.Theme)["Color Text"],Font=Enum.Font.Gotham,TextSize=10},f)
	local b=createInstance("TextBox",{Name="B",Text=tostring(math.floor(d.B*255)),Size=UDim2.new(0,30,0,20),Position=UDim2.new(0,75,0,25),BackgroundColor3=getCurrentTheme(AuraHub.Save.Theme)["Color Stroke"],TextColor3=getCurrentTheme(AuraHub.Save.Theme)["Color Text"],Font=Enum.Font.Gotham,TextSize=10},f)
...
function SectionMethods:addScriptBox(o)
	...
	addBorder(f,getCurrentTheme(AuraHub.Save.Theme)["Color Stroke"],1)
	local l=createInstance("TextLabel",{Name="Label",Text=n,Size=UDim2.new(1,0,0,20),Position=UDim2.new(0,0,0,0),BackgroundTransparency=1,TextColor3=getCurrentTheme(AuraHub.Save.Theme)["Color Text"],Font=Enum.Font.Gotham,TextSize=10},f)
	local tb=createInstance("TextBox",{Name="ScriptInput",Text=d,Size=UDim2.new(1,-10,0,40),Position=UDim2.new(0,5,0,25),BackgroundColor3=getCurrentTheme(AuraHub.Save.Theme)["Color Stroke"],TextColor3=getCurrentTheme(AuraHub.Save.Theme)["Color Text"],Font=Enum.Font.Code,TextSize=12,ClearTextOnFocus=false,MultiLine=true,TextWrapped=false},f)
	local runBtn=createInstance("TextButton",{Name="RunButton",Text="Run",Size=UDim2.new(0,40,0,20),Position=UDim2.new(1,-45,0,25),BackgroundColor3=getCurrentTheme(AuraHub.Save.Theme)["Color Theme"],TextColor3=getCurrentTheme(AuraHub.Save.Theme)["Color Text"],Font=Enum.Font.GothamBold,TextSize=10},f)
...
function SectionMethods:addBind(o)
	...
	addBorder(c,getCurrentTheme(AuraHub.Save.Theme)["Color Stroke"],1)
	local l=createInstance("TextLabel",{Name="Label",Text=n,Size=UDim2.new(0.7,0,1,0),Position=UDim2.new(0,0,0,0),BackgroundTransparency=1,TextColor3=getCurrentTheme(AuraHub.Save.Theme)["Color Text"],Font=Enum.Font.Gotham,TextSize=10,TextXAlignment=Enum.TextXAlignment.Left},c)
	local tb=createInstance("TextBox",{Name="BindInput",Text=d,Size=UDim2.new(0.3,0,1,0),Position=UDim2.new(0.7,0,0,0),BackgroundColor3=getCurrentTheme(AuraHub.Save.Theme)["Color Stroke"],TextColor3=getCurrentTheme(AuraHub.Save.Theme)["Color Text"],Font=Enum.Font.Gotham,TextSize=10},c)
	local currentKey
	local conn
...
function SectionMethods:addButton(o)
	...
	addBorder(c,getCurrentTheme(AuraHub.Save.Theme)["Color Stroke"],1)
	local l=createInstance("TextLabel",{Name="Label",Text=nm,Size=UDim2.new(0.7,0,1,0),Position=UDim2.new(0,0,0,0),BackgroundTransparency=1,TextColor3=getCurrentTheme(AuraHub.Save.Theme)["Color Text"],Font=Enum.Font.Gotham,TextSize=10,TextXAlignment=Enum.TextXAlignment.Left},c)
	local b=createInstance("TextButton",{Name="ActionButton",Text=bt,Size=UDim2.new(0.3,0,0.8,0),Position=UDim2.new(0.7,0,0.1,0),BackgroundColor3=getCurrentTheme(AuraHub.Save.Theme)["Color Theme"],TextColor3=getCurrentTheme(AuraHub.Save.Theme)["Color Text"],Font=Enum.Font.GothamBold,TextSize=10},c)
...
function AuraHub:addSection(sn, sh)
	sn = sn or "Section"
	sh = sh or 100
	local sf = createInstance("Frame", {
		Name = sn,
		Size = UDim2.new(1, 0, 0, sh),
		BackgroundColor3 = getCurrentTheme(AuraHub.Save.Theme)["Color Hub 2"],
		BackgroundTransparency = 0
	}, nil)
...
function AuraHub:CreateWindow(o)
	o = o or {}
	local wn = o.Name or "AuraHub Window"
	local th = getCurrentTheme(o.Theme)
	local subTitle = o.SubTitle or "SubTitle"
	local ft = wn .. " - " .. subTitle
	local pl = Players.LocalPlayer
	local pg = pl:WaitForChild("PlayerGui")
	local sg = createInstance("ScreenGui", {Name = "AuraHub", ResetOnSpawn = false}, pg)
	self.ScreenGui = sg
	local uw = AuraHub.Save.UISize[1]
	local uh = AuraHub.Save.UISize[2]
	...
	local sbWidth = AuraHub.Save.TabSize
	local sb = createInstance("Frame", {
		Name = "Sidebar",
...
function AuraHub:ShowLoadingScreen(d, cb)
	d = d or 5
	...
	local tl = createInstance("TextLabel", {Text = "AuraHub Library", Font = Enum.Font.SourceSansSemibold, TextSize = 20, TextColor3 = getCurrentTheme(AuraHub.Save.Theme)["Color Text"], BackgroundTransparency = 1, Size = UDim2.new(0, 200, 0, 50), AnchorPoint = Vector2.new(0.5, 0.5), Position = UDim2.new(0.5, 0, 0.5, -50)}, lg)
	local wl = createInstance("TextLabel", {Text = "Welcome, " .. pl.Name, Font = Enum.Font.SourceSansSemibold, TextSize = 16, TextColor3 = getCurrentTheme(AuraHub.Save.Theme)["Color Text"], BackgroundTransparency = 1, Size = UDim2.new(0, 200, 0, 30), AnchorPoint = Vector2.new(0.5, 0), Position = UDim2.new(0.5, 0, 0.5, 10)}, lg)
	local pbc = createInstance("Frame", {Size = UDim2.new(0.5, 0, 0, 20), Position = UDim2.new(0.5, 0, 0.5, 50), AnchorPoint = Vector2.new(0.5, 0.5), BackgroundColor3 = getCurrentTheme(AuraHub.Save.Theme)["Color Stroke"], BackgroundTransparency = 0.5, BorderSizePixel = 0}, lg)
	createInstance("UICorner", {CornerRadius = UDim.new(0, 4)}, pbc)
	local pbf = createInstance("Frame", {Size = UDim2.new(0, 0, 1, 0), BackgroundColor3 = getCurrentTheme(AuraHub.Save.Theme)["Color Theme"], BorderSizePixel = 0}, pbc)
...
function AuraHub:InitializeUI(o)
...
function AuraHub:SaveConfig(f)
	f = f or "AuraHub_Config.json"
	if writefile then
		local j = HttpService:JSONEncode(AuraHub.ConfigData)
		writefile(f, j)
...
function AuraHub:LoadConfig(f)
	f = f or "AuraHub_Config.json"
	if readfile then
		local d = readfile(f)
		local c = HttpService:JSONDecode(d)
		AuraHub.ConfigData = c
		AuraHub:ApplyConfig(c)
...
function AuraHub:UpdateTheme(nt)
	AuraHub.Save.Theme = nt
...
function AuraHub:SetTheme(nt)
	self:UpdateTheme(nt)
end

function AuraHub:ToggleUIVisibility()
	if self.ScreenGui and self.ScreenGui.Parent then
		self.ScreenGui.Enabled = not self.ScreenGui.Enabled
	end
end

function AuraHub:Notify(o)
	o = o or {}
	local t = o.Title or ""
	local m = o.Message or ""
	local d = o.Duration or 5
	local showProgress = o.ShowProgress or false
	local theme = getCurrentTheme(AuraHub.Save.Theme)
